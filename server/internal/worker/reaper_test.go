...
func TestReaper_ReclaimsAStuckSubmission(t *testing.T) {
	...
	reaped, err := worker.NewReaper(svc, time.Minute).Sweep(ctx)
...
func TestReaper_NeverOverwritesAVerdict(t *testing.T) {
	...
	ctx := context.Background()
	id := stale(t, svc, repo, "user-1", time.Hour)
	// MarkJudged is conditional on the submission being claimed, so the
	// verdict has to be recorded the way the pipeline records one.
	require.NoError(t, svc.MarkRunning(ctx, id))
...
func TestReaper_RunSweepsOnATickerAndStops(t *testing.T) {
	...
	go func() {
		worker.NewReaper(svc, 10*time.Millisecond).Run(ctx)
		close(done)
	}()

	require.Eventually(t, func() bool {
		stored, err := svc.GetByID(context.Background(), id)
		return err == nil && stored.Status == submission.StatusJudgeError
